...
describe('registerIpcRouter', () => {
    ...
    const testRouter = {
        getUser: t.input(z.object({})).output(z.object({ name: z.string() })).use(),
        greet: t.input(z.object({ name: z.string() })).output(z.string()).use(),
        adminOnly: t.input(z.object({})).output(z.void()).use(async ({ ctx }, next) => {
    ...
    it('should register ipc handler', () => {
        registerIpcRouter<typeof testRouter, typeof mockContext>('test-channel', testRouter, handlers, createContext)
    ...
    it('should handle successful request', async () => {
        ...
        const result = await handle!({}, { key: 'greet', input: { name: 'World' } })
    ...
    it('should return error for unknown key', async () => {
        ...
        expect(result).toEqual({ error: { code: 'INTERNAL' } })
    })
    ...
    it('should handle plugin hooks', async () => {
        ...
        registerIpcRouter<typeof testRouter, typeof mockContext>('test-channel', testRouter, handlers, createContext, [plugin])
        const handle = ipcHandlers.get('test-channel')
